...
class Exec(
    ...
    private val compiledLambdas: MutableMap<LambdaExpr, List<IRNode>> = HashMap(),
    private val loweredCases: MutableMap<CaseExpr, Expr> = HashMap(),
) {
    ...
    private fun shouldEmitLet(name: String): Boolean {
        val t = currentTracer()
        return t != null && (t.opts.step || name in t.opts.watch)
    }
    ...
    private fun handleArray(e: ArrayExpr, env: Env): List<Any?> {
        val out = ArrayList<Any?>(e.elements.size)
        for (element in e.elements) {
            out.add(evalExpr(element, env))
        }
        return out
    }
    ...
    private fun handleCase(e: CaseExpr, env: Env): Any? {
        val lowered = loweredCases.getOrPut(e) { lowerCaseExpr(e) }
        return evalExpr(lowered, env)
    }
    ...
    private fun handleLambda(e: LambdaExpr, env: Env): FnValue {
        val compiled: List<IRNode>? =
            (e.body as? BlockBody)?.let { body ->
                compiledLambdas.getOrPut(e) {
    ...
    private fun evalNumericBinary(e: BinaryExpr, left: Any?, right: Any?): Any {
        val site = numericSites.getOrPut(e) { NumericBinarySite(e.token.type) }
        return site.eval(left, right)
    }
    ...
    private fun handleLet(n: IRLet, env: Env) {
        ...
        val emit = shouldEmitLet(n.name)
        val old = if (emit) env.get(n.name) else null
        val new = evalExpr(n.expr, env)
        env.setOrDefine(n.name, new)
        if (emit) emitLet(n.name, old, new)
    }
    ...
    private fun canFastPathForEachBody(nodes: List<IRNode>): Boolean {
        for (n in nodes) {
            when (n) {
                is IRLet,
                is IRSet,
    ...
    private fun execObjectNoReturn(nodes: List<IRNode>, env: Env, out: MutableList<Map<Any, Any?>>) {
        for (n in nodes) {
            emitEnter(n)
            try {
                when (n) {
...
private fun createNumericVariant(
    ...
    return when (op) {
        TokenType.PLUS -> createAddVariant(leftKind, rightKind)
        TokenType.MINUS -> createSubVariant(leftKind, rightKind)
        TokenType.STAR -> createMulVariant(leftKind, rightKind)
        TokenType.SLASH -> createDivVariant(leftKind, rightKind)
        TokenType.SLASH_SLASH -> createIDivVariant(leftKind, rightKind)
        TokenType.PERCENT -> createRemVariant(leftKind, rightKind)
        else -> null
    }
}
...
private fun evalGenericNumeric(op: TokenType, left: Any?, right: Any?): Any = when (op) {
    TokenType.PLUS -> addNum(left, right)
    TokenType.MINUS -> subNum(left, right)
    TokenType.STAR -> mulNum(left, right)
    TokenType.SLASH -> divNum(left, right)
    TokenType.SLASH_SLASH -> idivNum(left, right)
    TokenType.PERCENT -> remNum(left, right)
    else -> error("Unsupported numeric op $op")
}
