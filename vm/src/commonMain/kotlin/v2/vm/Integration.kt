...
class VMEval(
    ...
    fun eval(expr: Expr, env: MutableMap<String, Any?>): Any? {
        return try {
            ...
            val fallback = v2.ir.makeEval(hostFns, funcs, reg, tracer)
            tracer?.on(v2.debug.TraceEvent.Call("FALLBACK", "Interpreter", listOf(expr::class.simpleName)))
            val out = fallback(expr, env)
            tracer?.on(v2.debug.TraceEvent.Return("FALLBACK", "Interpreter", out))
            out
        } catch (e: Exception) {
            // Log and fallback
            tracer?.on(v2.debug.TraceEvent.Error("VM expression evaluation failed: ${e.message}", e))
            val reg = TransformRegistry(funcs, hostFns, emptyMap())
            val fallback = v2.ir.makeEval(hostFns, funcs, reg, tracer)
            tracer?.on(v2.debug.TraceEvent.Call("FALLBACK", "Interpreter", listOf(expr::class.simpleName)))
