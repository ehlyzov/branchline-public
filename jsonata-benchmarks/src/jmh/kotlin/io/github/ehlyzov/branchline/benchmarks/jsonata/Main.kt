...
import io.github.ehlyzov.branchline.benchmarks.jsonata.test.Loader.filterCases
...
fun main(vararg args: String) {
    val engineId = args.getOrNull(0)?.trim().orEmpty().ifBlank { ENGINE_IBM }
    val caseFilter = args.getOrNull(1)?.trim().orEmpty().ifBlank {
        "numeric-operators/case000,partial-application/case002,performance/case001"
    }
    val cases = filterCases(CrossEngineCases.loadAll(), caseFilter)
    println("Engine: $engineId")
    println("Cases: " + cases.joinToString(", ") { it.id })
    for (case in cases) {
        println("Running: ${case.id}")
        val inputs = buildInputs(case)
        val prepared = prepareCase(case, inputs, engineId)
        if (prepared.errorMessage != null) {
            println("Error: ${prepared.errorMessage}")
            continue
        }
